...
void DynamixelClass::setID(unsigned char ID, unsigned char newID) {
	unsigned char arr[1] = { newID };
	writeN(ID, 0x07, arr, 1);
}
...
void DynamixelClass::setPosPgain(unsigned char ID, unsigned int pgain) {
	pgain %= 16383;
...
int DynamixelClass::getTemp(unsigned char ID) {
	clearRXbuffer();
	readN(ID, 0x92, 4);
	getParameters();
	int temp;
	temp = data[1];
	Serial.print("Temp of ID: ");
	Serial.print(data[0]);
	Serial.print(" is ");
	Serial.println(temp);
